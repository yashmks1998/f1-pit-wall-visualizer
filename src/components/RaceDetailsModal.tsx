  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4 bg-black/60 dark:bg-black/80 backdrop-blur-md">
      ...
      <motion.div
        ...
        className="w-full max-h-[90dvh] md:max-h-none md:h-auto md:max-w-4xl bg-white dark:bg-[#0a0a0a] rounded-t-[32px] md:rounded-3xl border-t md:border-t-0 md:border border-gray-200 dark:border-white/10 shadow-lg overflow-y-auto md:overflow-hidden relative z-10 p-0"
      >
        ...
        <div className="p-6 md:p-8">
          {/* Header */}
          <div className="flex flex-col md:flex-row md:justify-between md:items-start border-b border-gray-200 dark:border-white/10 pb-5 gap-4">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className={`text-[9px] font-extrabold px-2.5 py-1 rounded-md uppercase tracking-wider border ${
                  isCompleted ? 'bg-gray-50 dark:bg-white/5 border-gray-200 dark:border-white/10 text-gray-500 dark:text-gray-400' : 'bg-[#ff1801]/10 border-[#ff1801]/20 text-[#ff1801]'
                }`}>
                  {isCompleted ? 'Completed' : 'Upcoming Event'}
                </span>
              </div>
              <h3 className="text-xl md:text-2xl font-black text-gray-900 dark:text-gray-100 mt-2 font-display tracking-tight leading-tight uppercase">
                {race.raceName}
              </h3>
            ...
            <div className="flex items-center gap-3 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 p-3 rounded-xl self-start md:self-auto font-mono-numbers shadow-sm">
              <Calendar className="w-4 h-4 text-gray-500 dark:text-gray-400" />
              <div className="text-left">
                <span className="text-[8px] uppercase tracking-wider text-gray-500 dark:text-gray-400 font-bold block">Race Weekend Date</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mt-6">
              ...
              {isCompleted ? (
                <>
                  <div className="flex items-center gap-2 pb-1">
                    ...
                    <h4 className="font-black text-[10px] uppercase tracking-widest text-gray-900 dark:text-gray-100">Official Race Results</h4>
                  </div>

                  {loading ? (
                    <div className="h-64 flex flex-col items-center justify-center gap-3">
                      <div className="w-8 h-8 border-3 border-gray-200/20 dark:border-white/10 border-t-[#ff1801] rounded-full animate-spin"></div>
                      <span className="text-xs uppercase tracking-wider font-extrabold text-gray-500 dark:text-gray-400 font-mono-numbers">Loading Classification...</span>
                    </div>
                  ) : results.length > 0 ? (
                    <div className="table-container max-h-[350px] overflow-y-auto border border-gray-200 dark:border-white/10 rounded-2xl pr-1">
                      <table className="f1-table w-full">
                        <thead>
                          <tr className="border-b border-gray-200 dark:border-white/10 text-gray-500 dark:text-gray-400 bg-gray-50/50 dark:bg-white/5">
                            ...
                            <th className="cell-pts py-2.5 pr-4 text-xs font-bold uppercase tracking-wider text-right">Points</th>
                          </tr>
                        </thead>
                        <tbody>
                          {results.map(item => {
                            ...
                            return (
                              <tr 
                                ...
                                className={`border-b border-gray-200 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/5 transition-all ${
                                  isFastestLap ? 'bg-purple-950/5 dark:bg-purple-950/15' : ''
                                }`}
                              >
                                <td className="cell-pos py-3 pl-4 text-sm font-bold font-mono-numbers text-gray-900 dark:text-gray-100">
                                  P{item.position}
                                </td>
                                <td className="cell-team-accent w-1">
                                  <span className="w-1 h-5 block rounded-full" style={{ backgroundColor: colorInfo.color }}></span>
                                </td>
                                <td className="py-3 text-xs font-semibold text-gray-900 dark:text-gray-100">
                                ...
                                <td className="py-3 text-gray-500 dark:text-gray-400 text-xs font-medium">{item.Constructor.name}</td>
                                <td className="py-3 text-center text-xs font-mono-numbers text-gray-500 dark:text-gray-400">P{item.grid}</td>
                                <td className="py-3 text-center text-xs font-mono-numbers text-gray-500 dark:text-gray-400">{item.Time?.time || item.status}</td>
                    ...
                    <div className="text-center py-12 text-gray-500 dark:text-gray-400 bg-gray-50/40 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl">
                ...
                <>
                  ...
                  <div className="flex items-center gap-2 pb-1">
                    ...
                    <h4 className="font-black text-[10px] uppercase tracking-widest text-gray-500 dark:text-gray-400">Green Flag Countdown</h4>
                  </div>

                  <div className="grid grid-cols-4 gap-4 p-6 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl text-center shadow-sm">
                    {[
                      ...
                      <div key={t.lbl} className="flex flex-col gap-1.5">
                        <span className="text-2xl md:text-3xl font-extrabold text-gray-900 dark:text-gray-100 font-mono-numbers tracking-tight">{String(t.val).padStart(2, '0')}</span>
                        <span className="text-[8px] uppercase tracking-widest text-gray-500 dark:text-gray-400 font-bold">{t.lbl}</span>
                      </div>
                    ))}
                  </div>

                  {/* Circuit Wiki Summary */}
                  <div className="bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl p-5 mt-2 flex flex-col gap-2 shadow-sm">
                    <div className="flex items-center gap-2">
                      <Info className="w-4 h-4 text-[#ff1801]" />
                      <span className="text-[9px] uppercase tracking-widest text-gray-500 dark:text-gray-400 font-bold">Circuit Overview</span>
                    </div>
            ...
            <div className="col-span-1 md:col-span-4 flex flex-col gap-4">
              <div className="flex items-center gap-2 pb-1">
                ...
                <h4 className="font-black text-[10px] uppercase tracking-widest text-gray-500 dark:text-gray-400">Circuit Layout</h4>
              </div>

              <div className="bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl p-5 flex flex-col gap-4 shadow-sm">
                
                {/* Circuit specifications details list */}
                <div className="flex flex-col gap-3 font-semibold text-xs text-gray-500 dark:text-gray-400">
                  <div className="flex justify-between border-b border-gray-200 dark:border-white/10 pb-2">
                    <span>Track Length</span>
                    <span className="text-gray-900 dark:text-gray-100 font-mono-numbers">{spec?.length || '5.303 km'}</span>
                  </div>
                  <div className="flex justify-between border-b border-gray-200 dark:border-white/10 pb-2">
                    <span>Total Laps</span>
                    <span className="text-gray-900 dark:text-gray-100 font-mono-numbers">{spec?.laps || '58'}</span>
                  </div>
                  <div className="flex justify-between border-b border-gray-200 dark:border-white/10 pb-2">
                    <span>Total Turns</span>
                    <span className="text-gray-900 dark:text-gray-100 font-mono-numbers">{spec?.turns || '16'}</span>
                  </div>
                  <div className="flex justify-between pb-1">
                    <span>Lap Record</span>
                    <span className="text-gray-900 dark:text-gray-100 font-mono-numbers text-right">{spec?.lapRecord || '1:24.125 (Hamilton, 2020)'}</span>
                  </div>
                </div>

                {/* Technical Map Image */}
                <div className="w-full aspect-[4/3] rounded-xl overflow-hidden bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-white/10 flex items-center justify-center p-4 relative group">
                  <span className="text-[9px] text-gray-500 dark:text-gray-400 text-center font-bold absolute bottom-2 tracking-wide uppercase select-none">Layout Specs</span>
